...
async function fetchAllCards(setId: string, apiKey: string): Promise<Card[]> {
  const all: Card[] = [];
  const limit = 20; // free-tier cap
  let offset = 0;
  while (true) {
    const url = `https://api.justtcg.com/v1/cards?game=disney-lorcana&set=${encodeURIComponent(setId)}&limit=${limit}&offset=${offset}`;
...
async function main() {
  ...
  const ourSets = await sql<{ id: number; slug: string; set_code: string; justtcg_set_id: string }[]>`
    ...
    WHERE g.slug = 'lorcana' AND s.justtcg_set_id IS NOT NULL
      AND (
        ${resume} = false
  ...
  if (!resume) {
    console.log(`Wiping cards + prices (CASCADE)...`);
    await sql`TRUNCATE cards RESTART IDENTITY CASCADE`;
  } else {
  ...
  for (const set of ourSets) {
    const cards = await fetchAllCards(set.justtcg_set_id, apiKey);
    ...
    await sql.begin(async (tx) => {
      for (const c of cards) {
        ...
        const rows = await tx<{ id: number }[]>`
          ...
          VALUES (
            ${set.id}, ${c.number}, ${c.name}, ${c.rarity}, ${variantLabel},
            ${c.id}, ${v.id}, ${tcgpId},
            ${cents}, ${lastPriceAt}
          )
  ...
  const sample = await sql`
    ...
    LIMIT 10
  `;
  console.log(`\nTop 10 most expensive cards across all 12 sets:`);
